...
class FplApi:
    ...
    def combine_data(self):
        ...
        self.players = self.players[['id', 'first_name',
                                     'second_name', 'web_name',
                                     'element_type', 'team', 
                                     'now_cost']]
        player_position = pd.merge(self.players, 
                                   self.positions[['id', 
                                                   'singular_short_name']],
                                   left_on='element_type', 
                                   right_on='id')
        player_position = player_position[['id_x', 'first_name',
                                           'second_name', 'web_name',
                                           'team', 'team_code',
                                           'singular_name_short', 'now_cost']]
        self.combined_data = pd.merge(player_position, self.teams[['id', 'name']],
    ...
    def rename(self):
        ...
        rename = {'id_x': 'player_id', 'singular_name_short': 'POSITION',
                  'now_cost': 'COST', 'name': 'TEAM'}
        self.combined_data = self.combined_data.rename(columns=rename)
        self.combined_data['NAME'] = (self.combined_data['first_name'] 
                                      + ' ' + self.combined_data['second_name'])
