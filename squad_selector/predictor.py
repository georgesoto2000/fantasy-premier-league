...
class Predictor:
    ...
    def find_best_params(self,  train_x: np.array, train_y: np.array,
                         params: dict = {'learning_rate': [0.01, 0.05, 0.1],
                                         'n_estimators': [165, 185, 205]}) -> None:
        ...
        gd_search = GridSearchCV(model, params,
                                 scoring='neg_mean_absolute_error')
        gd_search.fit(train_x, train_y)
        self.log.info('Grid Search Complete')
        self.best_params = gd_search.best_params_
        self.model = GradientBoostingRegressor(learning_rate=self.best_params['learning_rate'],
    ...
    def build_model(self, test_size=0.2) -> None:
        ...
        encoded_data = encoded_data[['AGE', 'TEAM_RANK', 'POSITION_GK',
                                    'POSITION_DF', 'POSITION_MF', 'POSITION_FW', 
                                    'MATCHES_PLAYED', 'STARTS', 'MINUTES', 
                                    'GOALS', 'ASSISTS', 'PENALTIES', 
                                    'PEANLTIES_MISSED', 'YELLOW_CARDS', 
                                    'RED_CARDS', 'xG', 'xA', 'OWN_GOALS', 
                                    'GOALS_CONCEDED', 'CLEAN_SHEETS', 'SAVES', 
                                    'POINTS', 'MINUTES_PER_ASSIST', 
                                    'MINUTES_PER_GOAL', 'MINUTES_PER_GAME', 
                                    'MINUTES_PER_POINT', 'MINUTES_PER_XG', 
                                    'MINUTES_PER_XA', 'xg_diff', 'xa_diff', 
                                    'NEXT_SEASON_POINTS']]
        imputed_player_data = self.impute_data(encoded_data)
        train, test = train_test_split(imputed_player_data,
                                       test_size=test_size)
    ...
    def visualise_results(self, test_x: np.array, test_y: np.array) -> None:
        ...
        plt.scatter(x=[range(0, len(test_y), 1)], y=test_y,
                    label='True Values', marker='o')
        plt.scatter(x=[range(0, len(predicted_y), 1)], y=predicted_y, 
                    label='Pred Values', marker='o')
        true_avg = np.average(test_y)
        pred_avg = np.average(predicted_y)
        plt.axhline(y=true_avg, color='blue', 
                    linestyle='--', label=f'True Average:{true_avg:.2f}')
        plt.axhline(y=pred_avg, color='orange',
                    linestyle='--', label=f'Prediction Average:{pred_avg:.2f}')
    ...
    def predict(self, to_predict_data: pd.DataFrame) -> pd.DataFrame:
        ...
        encoded_data = encoded_data[['AGE', 'RK', 'POSITION_GK',
                                    'POSITION_DF', 'POSITION_MF', 'POSITION_FW', 
                                    'MATCHES_PLAYED', 'STARTS', 'MINUTES', 
                                    'GOALS', 'ASSISTS', 'PENALTIES', 
        ...
        predicted_data = to_predict_data[['NAME', 'RK', 'POSITION',
